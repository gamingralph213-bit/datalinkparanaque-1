...
export default function Home() {
  ...
  const joinedAbstractData = useMemo(() => {
    ...
    salesData.forEach(s => { if (s.arpNo) salesLookup.set(s.arpNo.trim().toUpperCase(), s); });
    ...
    const joined = journals.map(j => {
      ...
      const arpNorm = (j.arpNo || "").trim().toUpperCase();
      const salesMatch = salesLookup.get(arpNorm) || null;
